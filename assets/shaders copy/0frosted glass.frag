...
float fbm(vec2 pos) {
	...
	mat2 rot = mat2(cos(0.3), sin(0.5), -sin(0.5), cos(0.5));
...
void main(void) {
	vec2 p = (gl_FragCoord.xy * 6.7 - resolution.xy) / min(resolution.x, resolution.y);

	float t = 0.7, d;
	
	float time2 = time / 2.5;
	...
	float f = fbm(p + r);
	vec3 color = mix(
		vec3(1.101961, 0.619608, 1.666667),
