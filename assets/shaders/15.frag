...
#define iMouse vec4(.5 * resolution, 0., 0.)
...
float GetText(vec2 uv)
{
	...
	float d = 2000.0;
		...
		d = I(uv,d);uv.x -= 1.1;
		d = T(uv,d);
	return smoothstep(0.0,0.315,d-0.1255*CHS);
}
...
vec4 hash4(vec2 v)
{
    vec4 p = vec4(v, 0., 0.) * mat4( 127.1, 311.7, 0., 0.,
                                     269.5, 183.3, 0., 0.,
                                     113.5, 271.9, 0., 0.,
                                     246.1, 124.6, 0., 0. );
...
vec4 hash4(vec3 v)
{
    vec4 p = vec4(v, 0.) * mat4( 127.1, 311.7, 74.7, 0.,
                                 269.5, 183.3, 246.1, 0.,
                                 113.5, 271.9, 124.6, 0.,
                                 271.9, 269.5, 311.7, 0.);
    return fract(sin(p)*43758.5453123);
...
vec3 rain(vec3 ro3, vec3 rd3, float time) {
	...
	float d= 2.-abs(sin(time/2.))-GetText((rd3.yz+vec2(.2,0.))*vec2(1.,.7)*3.8);	
            ...
            if (tmin >= t2s && tmin <= t2) {
                ...
                if (abs(u) < target_rad) {
                    ...
                    if (v >= 0.0 && v < 1.0) {
                        ...
                        if (char_hash.x >= 0.1 || c == 0.) {  //10 percent of missed symbols
                            ...
                            if (a > 0.) {
                                ...
                                if (result.a > 0.98)  return result.xyz*d;
                            }
                        }
                    }
                }
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float level1_size = 2.*float(BLOCK_SIZE) * BLOCKS_BEFORE_TURN * XYCELL_SIZE;
    ...
    if (t8 < 0.125) {
       turn = dirL;
        ...
        vec2 mouse1 = iMouse.xy / iResolution.xy * vec2(4.,2.) - 1.;
        up_down = -0.7 * mouse1.y;
        angle += mouse1.x;
        rotate_on_turns = 1.;
        roll_on_turns = 0.;
